...
const eq = itemA => itemB => itemA === itemB
...
function callBeforeHooks(store, to, from, match) {
  const promises = match.matchedComponents.map(matchedComponent => {
    ...
    if (matchedComponent.$beforeRouteEnter) {
      return (
        matchedComponent.$beforeRouteEnter(store, to, from) || Promise.resolve()
      )
    }
